    ...
    public class UserAgentHelper : IUserAgentHelper
    {
        ...
        protected virtual BrowscapXmlHelper GetBrowscapXmlHelper()
        {
            ...
            lock (_locker)
            {
                ...
                var userAgentStringsPath = CommonHelper.MapPath("~/App_Data/browscap.xml");
                var crawlerOnlyUserAgentStringsPath = CommonHelper.MapPath("~/App_Data/browscap.crawlersonly.xml");
        ...
        public bool IsWeChat()
        {
            if (_httpContextAccessor?.HttpContext == null)
                return false;
            var userAgent = _httpContextAccessor.HttpContext.Request.Headers[HeaderNames.UserAgent].ToString();
            return !string.IsNullOrEmpty(userAgent) && userAgent.IndexOf("MicroMessenger", StringComparison.InvariantCultureIgnoreCase) >= 0;
        }
        ...
        public bool IsIphone()
        {
            if (_httpContextAccessor?.HttpContext == null)
                return false;
            var userAgent = _httpContextAccessor.HttpContext.Request.Headers[HeaderNames.UserAgent].ToString();
            return !string.IsNullOrEmpty(userAgent) && userAgent.IndexOf("iphone", StringComparison.InvariantCultureIgnoreCase) >= 0;
        }
